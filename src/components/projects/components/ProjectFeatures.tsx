...
export default function ProjectFeatures({ features }: ProjectFeaturesProps) {
  ...
  useEffect(() => {
    const observerOptions = {
      ...
      rootMargin: "-20% 0px -60% 0px", // triggers action when item is near top-center of view
      threshold: 0,
    };
      ...
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          if (categoryId) {
            ...
            // Optional: Auto-expand the item on scroll by uncommenting the line below
    ...
    // Scan DOM tree inside container wrapper tracking all layout blocks
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-10 lg:grid-cols-[240px_1fr] lg:gap-16">
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-32 space-y-4 border-l border-neutral-100 dark:border-neutral-900 pl-4">
              <div className="text-xs font-bold font-mono uppercase tracking-widest text-neutral-300 dark:text-neutral-700"></div>
              <nav
                className="space-y-3"
                ...
                {Object.keys(features).map((category) => {
                  ...
                  return (
                    <button
                      ...
                      className={`block w-full text-left text-xs font-bold font-mono uppercase tracking-wider transition-all duration-300 cursor-pointer relative pl-1 ${
                        isCurrentlyViewed
                    ...
                    >
                      {isCurrentlyViewed && (
                        <span className="absolute -left-4 text-black dark:text-white font-mono">
                          &gt;
                        </span>
                      )}
                      {category}
            ...
            {Object.entries(features).map(([category, items]) => {
              ...
              return (
                <div
                  ...
                  className={`py-2 first:pt-0 last:pb-0 transition-all duration-500 ease-out ${
                    isCurrentlyViewed
                      ? "opacity-100 bg-neutral-50/40 dark:bg-neutral-900/10 px-2 sm:px-4 rounded-sm"
                ...
                >
                  ...
                  <button
                    ...
                    className="group flex w-full items-center justify-between py-5 text-left transition-colors duration-150 cursor-pointer"
                    aria-expanded={isOpen}
                  >
                    <div className="flex items-center gap-4">
                      <span
                        className={`font-mono text-xs font-bold transition-transform duration-300 ${
                          isOpen
                            ? "rotate-90 text-black dark:text-white"
                            : "text-neutral-300 dark:text-neutral-700 group-hover:text-black dark:group-hover:text-white"
                        }`}
                      >
                        &gt;
                      </span>
                  ...
                  >
                    <div className="overflow-hidden pl-6">
                      <ul className="grid grid-cols-1 gap-x-8 gap-y-3.5 sm:grid-cols-2 font-sans text-sm">
                        {items.map((feature) => (
                          <li
                            key={feature}
